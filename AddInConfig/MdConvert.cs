    ...
    public static class MdConvert
    {
        ...
        public static string ToDateTimeString(object value, string format = "yyyy-MM-dd", bool isSimple = false)
        {
            ...
            if (DateTime.TryParse(value.ToString(), out result))
            {
                ...
                {
                    if (DateTime.Now.Year > result.Year || DateTime.Now.Month > result.Month || (DateTime.Now.Day - 1) > result.Day)
                    {
                        return result.ToString("yyyy-MM-dd " + format);
                    }
                    else
                    {
                        if (result.Year > DateTime.Now.Year || result.Month > DateTime.Now.Month || result.Day > DateTime.Now.Day)
                        {
                            return result.ToString("yyyy-MM-dd " + format);
                        }
